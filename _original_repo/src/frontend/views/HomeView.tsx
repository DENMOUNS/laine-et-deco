...
import { ArrowRight, Package, Truck, ShieldCheck, Heart, Calendar, User, Search, Camera, Zap, Clock, Loader2, Mic, X as CloseIcon } from 'lucide-react';
    ...
    <div className="space-y-24 pb-24">
          ...
          <div className="max-w-3xl text-white">
            ...
            <div className="max-w-3xl mb-12 relative z-50" ref={searchContainerRef}>
              <form onSubmit={handleSearch} className="relative group">
                {/* ... search input ... */}
                <div className={`absolute left-6 top-1/2 -translate-y-1/2 transition-colors z-10 ${isSearchFocused ? 'text-accent' : 'text-white/50'}`}>
                  <Search size={24} />
                </div>
                <input
                  type="text"
                  ...
                  className={`w-full backdrop-blur-2xl border rounded-full py-4 sm:py-6 pl-14 sm:pl-16 pr-32 sm:pr-48 text-base sm:text-lg focus:outline-none transition-all shadow-2xl relative z-0 ${
                    isSearchFocused 
                      ? 'bg-white/95 border-white text-primary placeholder:text-primary/40' 
                  ...
                  >
                    ...
                    <form onSubmit={handleTrackOrder} className="flex gap-2 mb-4">
                      <input 
                        ...
                        className="flex-grow bg-white/10 border border-white/20 rounded-full px-4 py-2 text-white placeholder:text-white/50 focus:outline-none focus:border-accent text-sm"
                      />
                      <button 
                        type="submit" 
                        ...
                        >
                          {trackingStatus.steps.map((step, index) => (
                            <div key={index} className="flex items-start gap-3">
                              ...
                              <div>
                                <p className={`text-sm font-bold ${step.completed ? 'text-white' : 'text-white/50'}`}>{step.label}</p>
              ...
              <AnimatePresence>
                {isSearchFocused && (
                  ...
                  >
                    <button 
                      onClick={() => setIsSearchFocused(false)}
                      className="absolute top-4 right-4 text-primary/30 hover:text-primary transition-colors z-10"
                    >
                      <CloseIcon size={20} />
                    </button>
                            ...
                            {liveSearchResults.map(product => (
                              ...
                              >
                                ...
                                <div className="flex-grow">
                                  ...
                                  <p className="text-[10px] text-primary/40 uppercase tracking-widest font-bold mt-1">{product.category}</p>
                                  <p className="text-sm font-serif italic text-accent mt-1">{product.price}€</p>
                                </div>
                                <ArrowRight size={18} className="text-primary/20 group-hover:text-accent group-hover:translate-x-1 transition-all" />
                              </button>
                            ))}
                            <button 
                      ...
                      <div className="flex-1">
                        ...
                        >
                          {isAnalyzingImage ? (
                            ...
                            <>
                              <div className="w-12 h-12 bg-white rounded-full shadow-sm flex items-center justify-center text-primary/40 group-hover:text-accent group-hover:scale-110 transition-all">
                                <Camera size={24} />
                              </div>
                              <span className="text-sm font-bold text-primary/60 group-hover:text-accent">Importer une photo</span>
                            </>
                          )}
                        </button>
                      </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {PRODUCTS.filter(p => p.isElectronic).map((product) => (
            <ProductCard 
              key={product.id}
              product={product} 
