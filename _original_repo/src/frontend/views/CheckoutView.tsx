...
export const CheckoutView: React.FC<CheckoutViewProps> = ({ cart, user, onNavigate, onComplete, allProducts }) => {
  ...
  const OrderSummary = () => (
    <div className="bg-white p-10 rounded-[3rem] border border-primary/5 shadow-sm">
            ...
            <div key={i} className="space-y-3">
                ...
                <div className="flex-grow">
                  ...
                  {item.type === 'pack' && (
                    <div className="flex flex-col gap-1 mt-1">
                      ...
                      <span className="text-[8px] font-bold text-primary/30 uppercase tracking-widest flex items-center gap-1">
              ...
              {item.type === 'pack' && packProducts && packProducts.length > 0 && (
                <div className="ml-20 space-y-1.5 pt-2 border-t border-primary/5">
                  <p className="text-[9px] font-bold text-primary/40 uppercase tracking-widest mb-1">Contenu :</p>
                  {packProducts.map((p, idx) => {
                    const productDetail = allProducts.find(prod => prod.id === p.productId);
                    return (
                      <div key={idx} className="flex justify-between items-center text-[10px]">
                        <span className="text-primary/60 font-medium">• {productDetail?.name || 'Produit inconnu'}</span>
                        <span className="text-primary/40">x{p.quantity || 1}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      {/* Promo Code */}
      <div className="mb-10">
        <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase mb-2 block">Code Promo</label>
      ...
      <div className="space-y-4 pt-4 border-t border-primary/5">
        ...
        <div className="flex items-center gap-3 text-xs font-medium text-primary/40">
          <Truck size={16} />
              ...
              <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg transition-all ${
                s.id === step 
                  ...
                  : s.id < step 
                    ...
                    : 'bg-white border border-primary/10 text-primary/30'
              }`}>
                {s.id}
              </div>
              <span className={`text-[10px] font-bold tracking-[0.2em] transition-colors ${
                s.id === step ? 'text-accent' : 'text-primary/30'
              }`}>
                {s.label}
              </span>
            </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase">Prénom</label>
                    <input 
                      type="text" 
                      placeholder="Jean" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase">Nom</label>
                    <input 
                      type="text" 
                      placeholder="Dupont" 
                  ...
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase">Adresse</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase">Ville</label>
                    <div className="relative">
                      ...
                      <div className="absolute inset-y-0 right-6 flex items-center pointer-events-none text-primary/40">
                        <svg width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1.5L6 6.5L11 1.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase">Téléphone</label>
                    <input 
                      type="tel" 
                  ...
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[10px] font-bold tracking-widest text-primary/40 uppercase">Coordonnées GPS</label>
                    <input 
                      type="text" 
                      readOnly
                      placeholder="Position non définie" 
                      value={formData.coordinates} 
                      className="w-full px-6 py-4 rounded-2xl bg-primary/5 border border-primary/10 text-primary/40 cursor-not-allowed font-mono text-xs" 
                    />
                  </div>
                </div>
                ...
                <div className="bg-primary/5 p-8 rounded-[2.5rem] flex items-center justify-between border border-primary/10">
                  <div className="flex gap-4 items-center">
                    <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-sm text-primary/40">
                      <MapPin size={24} />
                    </div>
                    <div>
                      <h4 className="font-serif font-bold text-primary">Géolocalisation pour la livraison</h4>
                      <p className="text-sm text-primary/40">Partagez votre position pour une livraison plus précise</p>
                    </div>
                  </div>
                  <button 
                  ...
                  {[
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        ...
                        <p className="text-xs text-primary/40">{method.description}</p>
                      </div>
                      <method.icon className={formData.paymentMethod === method.id ? 'text-accent' : 'text-primary/20'} />
                    </label>
                  ))}
                </div>
                ...
                {!user ? (
                  ...
                  <div className="bg-white p-10 rounded-[3rem] border border-primary/5 shadow-sm space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div>
                        <h4 className="text-[10px] font-bold tracking-widest text-primary/40 uppercase mb-4">Livraison</h4>
                        ...
                        <p className="text-primary/60">{formData.city}</p>
                      ...
                      <div>
                        <h4 className="text-[10px] font-bold tracking-widest text-primary/40 uppercase mb-4">Contact</h4>
                        <p className="text-primary/60">{formData.phone}</p>
                        <p className="text-primary/60">{user?.email}</p>
                      </div>
                      <div>
                        <h4 className="text-[10px] font-bold tracking-widest text-primary/40 uppercase mb-4">Paiement</h4>
                        <p className="text-primary/60">{formData.paymentMethod === 'delivery' ? 'Paiement à la livraison' : formData.paymentMethod}</p>
                      </div>
                    </div>
                  </div>
                )}
