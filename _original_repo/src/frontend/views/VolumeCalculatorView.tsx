...
export const VolumeCalculatorView: React.FC<VolumeCalculatorViewProps> = ({ onNavigate, onAddToCart }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 min-h-screen">
      <div className="flex items-center gap-2 text-sm text-primary/60 mb-8 font-bold tracking-widest uppercase">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-primary/10 shadow-xl shadow-primary/5 space-y-10">
            <div className="space-y-4">
              <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40 block">Type de matière</label>
            ...
            <div className="space-y-6">
              <div className="flex bg-primary/5 rounded-xl p-1 mb-4 w-full">
                <button 
                  ...
                  className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-lg transition-all ${calcMode === 'slider' ? 'bg-white shadow text-[#e26d24]' : 'text-primary/60 hover:text-primary'}`}
                >
                  Saisie manuelle
                </button>
                <button 
                  onClick={() => setCalcMode('dimensions')} 
                  className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-lg transition-all ${calcMode === 'dimensions' ? 'bg-white shadow text-[#e26d24]' : 'text-primary/60 hover:text-primary'}`}
                >
                  Par dimensions
                </button>
              </div>
              ...
              {calcMode === 'slider' ? (
                <>
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40">Volume du moule (ML)</label>
                  ...
                  {geoShape === 'rectangle' ? (
                    <div className="grid grid-cols-3 gap-3">
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40 ml-1">Long (cm)</label>
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40 ml-1">Larg (cm)</label>
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40 ml-1">Haut (cm)</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40 ml-1">Rayon (cm)</label>
            ...
            {/* Mod Sélection */}
            <div className="space-y-4">
              <label className="text-[10px] font-bold uppercase tracking-widest text-primary/40 block">Ou sélectionnez un de nos moules</label>
        ...
        >
          ...
          <div className="flex flex-col items-center mb-12">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 mb-2">Dosage Recommandé</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
              ...
              <div className="text-[10px] font-bold uppercase tracking-widest text-white/50">{material.part1Name}</div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center text-center">
              ...
              <div className="text-[10px] font-bold uppercase tracking-widest text-white/50">{material.part2Name}</div>
            </div>
          </div>

          <div className="border-t border-white/10 pt-6 pb-10 space-y-4">
            <div className="flex justify-between items-center text-sm">
              <span className="text-white/60">Volume Total</span>
              <span className="font-bold text-white">{volume} ml</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-white/60">Poids Total Est.</span>
              <span className="font-bold text-white">{totalWeight} g</span>
            </div>
          </div>

          <div className="space-y-4 mb-8">
            <p className="text-[10px] font-bold uppercase tracking-widest text-white/50 text-center">Produits Recommandés</p>
            <div className="bg-white/10 border border-white/20 rounded-2xl p-4 flex items-center justify-between cursor-pointer hover:bg-white/20 transition-colors" onClick={handleOrder}>
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <div className="text-white/60 text-xs">25 000 FCFA</div>
                 </div>
               </div>
          ...
          <div className="flex justify-center">
            <button onClick={reset} className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/50 hover:text-white transition-colors">
