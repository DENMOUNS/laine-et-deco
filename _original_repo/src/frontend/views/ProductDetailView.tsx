      ...
      <div className="bg-card p-8 md:p-12 rounded-[3rem] border border-primary/5 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="relative">
            <img src={product.image} alt={product.name} className="w-full h-auto rounded-3xl" referrerPolicy="no-referrer" />
            {product.isSale && (
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              {product.oldPrice && (
                <p className="text-xl text-primary/40 line-through">{product.oldPrice.toLocaleString()} FCFA</p>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {recommendedProducts.map(p => (
              <div key={p.id} className="bg-card p-6 rounded-[2rem] border border-primary/5 shadow-sm">
                <img src={p.image} alt={p.name} className="w-full h-64 object-cover rounded-2xl mb-4" referrerPolicy="no-referrer" />
