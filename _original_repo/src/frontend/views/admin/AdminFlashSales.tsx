        ...
        <div className="bg-card p-8 rounded-[2rem] border border-primary/10 shadow-sm max-w-4xl mx-auto">
          <form onSubmit={handleSave} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-primary/60 mb-2 block">Nom de la vente flash</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-primary/60 mb-2 block">Date de fin</label>
                <input 
                  type="datetime-local" 
                  className="input-field"
            ...
            <div className="border-t border-primary/10 pt-8 mt-8">
              ...
              <div className="mb-6">
                <label className="text-xs font-bold uppercase tracking-widest text-primary/60 mb-2 block">Ajouter un produit</label>
                <select 
                  className="input-field"
                  onChange={e => {
                    if (e.target.value) {
              ...
              <div className="space-y-4">
                    ...
                    <div key={item.productId} className="flex flex-col md:flex-row gap-4 items-center p-4 bg-secondary/20 rounded-xl border border-primary/5">
                      <div className="flex-grow flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-primary/60">Prix normal: {product.price} FCFA</p>
                        </div>
                      </div>
                      <div className="flex flex-wrap items-center gap-4">
                        <div>
                          <label className="text-[10px] uppercase text-primary/60 block">Prix Flash (FCFA)</label>
                          <input 
                            type="number" 
                        ...
                        <div>
                          <label className="text-[10px] uppercase text-primary/60 block">Quantité max</label>
                          <input 
                            type="number" 
                        ...
                        <button 
                          ...
                          className="p-2 text-primary/40 hover:text-red-500 mt-4"
                        >
                          <Trash2 size={16} />
                        </button>
                      </div>
                ...
                {(!editingItem?.items || editingItem.items.length === 0) && (
                  <p className="text-primary/40 text-sm italic text-center py-4">Aucun produit ajouté</p>
                )}
              </div>
            </div>
