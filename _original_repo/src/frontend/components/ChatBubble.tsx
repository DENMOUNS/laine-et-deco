...
export const ChatBubble: React.FC = () => {
  ...
  const sendMessage = async () => {
    ...
    if (!apiKey) {
      ...
      return "Désolé, l'assistant IA n'est pas configuré. Veuillez contacter l'administrateur.";
    }
    ...
    try {
      ...
      if (error.message?.includes('API_KEY_INVALID')) {
        return "Clé API invalide. L'assistant ne peut pas répondre pour le moment.";
      }
                ...
                return (
                  <div key={msg.id} className={`flex ${isAI ? 'justify-start' : 'justify-end'}`}>
                    <div className={`max-w-[85%] p-4 rounded-2xl text-sm shadow-sm ${
                      ...
                      <span className={`text-[10px] mt-2 block font-medium ${isAI ? 'text-primary/40' : 'text-white/60'}`}>
                        {formattedTime}
                      </span>
                    </div>
                  </div>
