...
export const TopMarquee: React.FC<TopMarqueeProps> = ({ siteConfig }) => {
  const fallbackItems = [
    { id: '1', text: "LIVRAISON OFFERTE DÈS 200 000 FCFA", iconName: "Package" },
    { id: '2', text: "NOUVELLE COLLECTION DISPONIBLE", iconName: "Sparkles" },
    { id: '3', text: "TRICOTÉ AVEC AMOUR", iconName: "Heart" },
  ];
  const baseItems = siteConfig?.marqueeItems?.length ? siteConfig.marqueeItems : fallbackItems;
        ...
        {[0, 1].map((setIndex) => (
          <div key={setIndex} className="flex gap-16 items-center pr-16 whitespace-nowrap">
            {items.map((item, index) => {
              const Icon = (LucideIcons as any)[item.iconName || 'Star'] || LucideIcons.Star;
              return (
