        ...
        <div className="space-y-8">
                        ...
                        {woolCalculations.map(calc => (
                            <div key={calc.id} className="bg-white p-6 rounded-[2rem] border border-primary/5 shadow-sm relative group">
                                ...
                                <div className="flex justify-between items-start mb-4 pr-8">
                                    ...
                                    <span className="text-xs text-primary/40">{calc.date}</span>
                                </div>
                                <div className="space-y-2 mb-4">
                                    <p className="text-sm text-primary/60">Taille: <span className="font-bold text-primary">{calc.size || 'N/A'}</span></p>
                                    <p className="text-sm text-primary/60">Laine: <span className="font-bold text-primary">{calc.yarnWeight}</span></p>
                                </div>
                                <div className="pt-4 border-t border-primary/5">
                                    <p className="text-center text-2xl font-serif font-bold text-primary">{calc.result} <span className="text-sm font-sans font-normal text-primary/40">pelotes</span></p>
                                </div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <p className="text-primary/40 italic text-sm">Aucun calcul de laine enregistré.</p>
                )}
            </div>
            ...
            <div>
                ...
                {volumeCalculations.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {volumeCalculations.map(calc => (
                            <div key={calc.id} className="bg-white p-6 rounded-[2rem] border border-primary/5 shadow-sm relative group">
                                ...
                                <div className="space-y-2 mb-4">
                                    <p className="text-sm text-primary/60">Matériau: <span className="font-bold text-primary capitalize">{calc.material}</span></p>
                                    <p className="text-sm text-primary/60">Volume: <span className="font-bold text-primary">{calc.volume} ml</span></p>
                                </div>
                                <div className="pt-4 border-t border-primary/5 grid grid-cols-2 gap-2 text-center">
                                    <div>
                                        <p className="text-[10px] text-primary/40 uppercase">Partie A</p>
                                        <p className="font-bold text-primary">{calc.details.partA}g</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] text-primary/40 uppercase">Partie B</p>
                    ...
                    <p className="text-primary/40 italic text-sm">Aucun calcul de volume enregistré.</p>
                )}
            </div>
        </div>
    </div>
