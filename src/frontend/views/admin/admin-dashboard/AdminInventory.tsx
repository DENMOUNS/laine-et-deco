  ...
  return (
    <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-card p-8 rounded-[2rem] border border-primary/10 shadow-sm">
                ...
                <p className="text-3xl font-serif font-bold text-primary">
                  {localProducts.reduce((acc, p) => acc + ((Number(p.price) || 0) * (Number(p.stock) || 0)), 0).toLocaleString()} FCFA
                </p>
              </div>
            </div>
            ...
            <div className="bg-card rounded-[2rem] shadow-sm border border-primary/10 overflow-hidden">
              ...
              onRowClick={(p) => { setEditingItem(p); setActiveTab('inventory-detail'); }}
                columns={[
                  ...
                  { header: 'Prix', accessor: (p: any) => `${Number(p.price || 0).toLocaleString()} FCFA`, sortable: true, sortKey: 'price' },
      ...
      {activeTab === 'inventory-detail' && editingItem && (
        <AdminInventoryDetail product={editingItem} ctx={ctx} />
      )}
    </>
  );
}

// Composant détail inventaire avec historique
function AdminInventoryDetail({ product, ctx }: { product: any; ctx: any }) {
  const { formatDate, setActiveTab, setEditingItem } = ctx;
  const { data: stockTransactions = [] } = useEntity<any>('stock_transaction', []);

  const stockMovements = stockTransactions
    .filter((movement: any) => movement.productId === product.id)
