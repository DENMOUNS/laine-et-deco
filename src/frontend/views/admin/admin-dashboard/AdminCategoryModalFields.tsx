...
export function AdminCategoryModalFields({ ctx }: { ctx: any }) {
  ...
  const handleTranslate = async (e: React.MouseEvent) => {
    const parent = (e.currentTarget.closest('.space-y-6') as HTMLElement);
    const frNameInput = (parent?.querySelector('input[name="name"]') as HTMLInputElement)?.value || editingItem?.name;
    if (!frNameInput) {
