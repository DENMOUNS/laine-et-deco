...
import { Plus, Wrench, CheckCircle2, Trash2, Edit } from 'lucide-react';
...
export function AdminMaintenanceConfig({ ctx }: { ctx: any }) {
  const { data: configs, createEntity, updateEntity, deleteEntity } = useEntity<MaintenanceModeConfig>('maintenance_config_history');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<MaintenanceModeConfig | null>(null);
  const [formData, setFormData] = useState({ isActive: false, message: '', endDate: '' });
  ...
  const handleSetActive = async (config: MaintenanceModeConfig) => {
    try {
      const activeConfig = configs.find(c => c.status === 'active');
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-card p-6 rounded-3xl border border-primary/10">
        <div className="flex items-center gap-4">
          <div className="p-3 bg-primary/5 text-primary rounded-2xl"><Wrench size={24} /></div>
          <div>
            <h2 className="text-2xl font-serif text-primary">Mode Maintenance</h2>
            <p className="text-sm text-primary/60">Gérez l'historique des mises en maintenance</p>
          </div>
        </div>
        <button onClick={() => { setEditingItem(null); setFormData({ isActive: false, message: '', endDate: '' }); setIsModalOpen(true); }} className="bg-primary text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-accent transition-all">
      ...
      <div className="bg-card rounded-3xl border border-primary/10 shadow-sm overflow-hidden">
        <DataTable
          data={configs || []}
          columns={[
            { header: 'Mode', accessor: (c: MaintenanceModeConfig) => c.isActive ? 'Activé' : 'Désactivé' },
            { header: 'Message', accessor: 'message' },
            { header: 'Statut', accessor: (c: MaintenanceModeConfig) => <StatusBadge status={c.status} /> },
            {
              header: 'Actions',
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-card w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden relative z-10 p-8 border border-primary/10">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <label className="flex items-center gap-3 p-4 border border-primary/10 rounded-xl cursor-pointer">
                ...
                <span className="text-sm font-bold text-primary">Activer le mode maintenance</span>
              </label>
              ...
              <div><label className="text-xs font-bold uppercase tracking-widest text-primary/60 mb-2 block">Date de fin (Optionnel)</label><input type="datetime-local" value={formData.endDate} onChange={e => setFormData({...formData, endDate: e.target.value})} className="w-full bg-secondary/50 border border-primary/10 rounded-xl px-4 py-3" /></div>
