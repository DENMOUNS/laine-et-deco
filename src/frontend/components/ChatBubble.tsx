...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ startOpen = false }) => {
  ...
  return (
    <div className="fixed bottom-20 right-3 sm:bottom-6 sm:right-6 z-[80] flex flex-col items-end pointer-events-none">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            role="dialog"
            ...
            className="pointer-events-auto bg-white/95 dark:bg-[#1a1d1a]/95 backdrop-blur-2xl rounded-[2rem] shadow-2xl border-none w-[calc(100vw-1.5rem)] sm:w-96 max-h-[75vh] sm:max-h-[85vh] overflow-hidden mb-3 flex flex-col"
          >
            {/* Header */}
            ...
            <div 
              ref={scrollRef}
              className="h-80 overflow-y-auto p-5 space-y-4 bg-secondary/10 dark:bg-black/20 scroll-smooth"
            >
              {messages.map((msg) => {
      ...
      <motion.button
        ...
        className={`pointer-events-auto p-3.5 sm:p-4 rounded-full shadow-xl transition-all duration-300 group relative flex items-center justify-center ${
          isOpen 
            ? 'bg-primary text-white shadow-primary/30' 
            : 'glass-ios text-primary dark:text-white'
        }`}
      >
        {!isOpen && (
