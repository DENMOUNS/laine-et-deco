    ...
    <div className="bg-card rounded-[3rem] shadow-sm border border-primary/5 p-10 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        <div className="space-y-4">
          <label className="text-xs font-bold uppercase tracking-widest text-primary/70 block">Nom Complet</label>
          <div className="relative">
            <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-primary/70" size={18} />
            <input 
                type="text" 
                name="name"
                value={formData.name} 
        ...
        <div className="space-y-4">
          <label className="text-xs font-bold uppercase tracking-widest text-primary/70 block">Email</label>
          <div className="relative">
            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-primary/70" size={18} />
            <input 
                type="email" 
                name="email"
        ...
        <div className="space-y-4">
          <label className="text-xs font-bold uppercase tracking-widest text-primary/70 block">Téléphone</label>
          <div className="relative">
            <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-primary/70" size={18} />
            <input 
                type="tel" 
                name="phone"
        ...
        <div className="space-y-4">
          <label className="text-xs font-bold uppercase tracking-widest text-primary/70 block">Adresse</label>
          <div className="relative">
            <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-primary/70" size={18} />
            <input 
                type="text" 
                name="address"
      ...
      <div className="pt-12 border-t border-primary/5 space-y-8">
        ...
        {savedPatterns.length > 0 ? (
              ...
              <div key={pattern.id} className="bg-secondary p-6 rounded-[2rem] border border-primary/5 space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-primary/70 uppercase tracking-widest mt-1">{pattern.createdAt}</p>
                  </div>
                  <Button 
                    variant="ghost"
                    size="icon"
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="px-3 py-1 bg-white border border-primary/10 rounded-full text-[10px] font-bold uppercase tracking-widest text-primary/70">
                    {pattern.projectType}
                  </span>
                  <span className="px-3 py-1 bg-white border border-primary/10 rounded-full text-[10px] font-bold uppercase tracking-widest text-primary/70">
                    {pattern.skillLevel}
          ...
          <div className="text-center py-12 bg-secondary rounded-[2rem] border border-dashed border-primary/10">
            <FileText className="mx-auto text-primary/70 mb-4" size={48} />
            <p className="text-primary/70 font-serif italic">Vous n'avez pas encore enregistré de patron.</p>
          </div>
        )}
      </div>
    </div>
