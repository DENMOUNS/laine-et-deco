...
interface NewsletterPopupProps {
  config: SiteConfig['newsletterPopup'];
}

export const NewsletterPopup: React.FC<NewsletterPopupProps> = ({ config }) => {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-primary/40 backdrop-blur-sm">
          ...
          >
            <Button 
              ...
              size="icon"
              onClick={handleClose}
              className="absolute top-6 right-6 z-10 p-2 bg-white/80 backdrop-blur-md rounded-full text-primary hover:bg-accent hover:text-white shadow-sm"
            >
              <X size={20} />
            </Button>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="relative">
                      <input
                        ...
                        required
                      />
                      <Mail className="absolute right-6 top-1/2 -translate-y-1/2 text-primary/70" size={20} />
                    </div>
                    <Button 
                      type="submit"
                      className="w-full py-4 rounded-2xl font-bold shadow-lg shadow-primary/20"
                    >
                      S'abonner & Profiter
                    </Button>
                  </form>
                  <p className="mt-6 text-[10px] text-primary/70 text-center">
