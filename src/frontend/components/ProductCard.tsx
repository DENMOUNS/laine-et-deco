...
export const ProductCard: React.FC<ProductCardProps> = React.memo(({ 
  ...
  return (
    <motion.div 
      ...
      className={`group relative bg-white md:clay-tactile rounded-2xl sm:rounded-3xl border border-primary/5 overflow-hidden shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col h-full ${
        isOutOfStock ? 'opacity-75 grayscale-[0.3]' : ''
      }`}
    >
      {/* Badges */}
      <div className="absolute z-10 flex flex-col gap-1 sm:gap-1.5 top-2.5 left-2.5 sm:top-3.5 sm:left-3.5">
        {product.isNew && (
      ...
      {/* Image Container */}
      <div 
        className="relative overflow-hidden cursor-pointer group/img w-full aspect-square sm:aspect-[4/3]" 
      ...
      >
        ...
        {/* Overlay actions (Wishlist, QuickView, Share, Compare - PC only for Compare) */}
        <div className="absolute inset-0 bg-black/20 opacity-0 group-hover/img:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-1.5 sm:gap-2 p-2">
          <motion.button 
            ...
            className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-white/90 text-primary flex items-center justify-center hover:bg-accent hover:text-primary transition-colors shadow-lg cursor-pointer"
          ...
          {/* Comparateur - UNIQUEMENT SUR PC (hidden md:flex) */}
          {onAddToComparison && (
            <motion.button 
              ...
              className="hidden md:flex w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/90 text-primary items-center justify-center hover:bg-accent hover:text-primary transition-colors shadow-lg cursor-pointer"
      ...
      {/* Content */}
      <div className="flex flex-col justify-between flex-grow min-w-0 p-2.5 sm:p-4 md:p-5">
        <div>
          ...
          <h3 
            className="font-serif text-primary group-hover:text-accent transition-colors mb-2 cursor-pointer font-semibold text-xs sm:text-sm md:text-base leading-snug line-clamp-3 sm:line-clamp-2 min-h-[2.4rem]" 
            onClick={() => onClick(product)}
            title={displayName}
          >
            {displayName}
        ...
        {/* Footer Row: Price + Add to Cart Icon Button */}
        <div className="flex items-center justify-between gap-1.5 pt-2 border-t border-primary/5 mt-auto">
          <div className="flex flex-col min-w-0">
            {hasDiscount && (
              <span className="text-[9px] sm:text-xs text-primary/50 line-through font-medium whitespace-nowrap">
          ...
          <button
            ...
            className="p-2 sm:p-2.5 bg-primary hover:bg-accent text-white rounded-xl transition-all shadow-xs hover:shadow-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
